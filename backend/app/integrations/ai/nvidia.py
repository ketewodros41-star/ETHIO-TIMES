...
interface using NVIDIA NIM OpenAI-compatible API.
"""
...
class NvidiaTextProvider(AIProvider):
    name: str = "nvidia"
    ...
    def generate_json(self, request: TextGenerationRequest) -> dict[str, Any]:
        if not self.is_available():
            raise ProviderNotConfiguredError("NVIDIA API key not configured")

        system_content = request.system or "You are an expert news analyst and journalist. Respond ONLY with valid, parseable JSON."
        if request.response_schema:
            schema_str = json.dumps(request.response_schema)
            system_content += f"\nYour response must strictly conform to this JSON schema: {schema_str}\nOutput ONLY the JSON object, nothing else."
        ...
        try:
            ...
            raise ProviderResponseError(f"NVIDIA API call failed: {exc}") from exc
