...
@router.get("/debug-translation-test")
def debug_translation_test_endpoint() -> dict[str, Any]:
    """Diagnostic endpoint to inspect live connectivity to AI providers on Render."""
    import time
    import httpx
    ...
    ar_key = getattr(settings, "agent_router_key", None)
    if not ar_key:
    ...
    if not nv_key:
        results["nvidia"] = {"configured": False}
    else:
        t0 = time.time()
        try:
            with httpx.Client(timeout=10.0) as client:
                res = client.get("https://integrate.api.nvidia.com/v1/models", headers={"Authorization": f"Bearer {nv_key}"})
                results["nvidia"] = {
                    "configured": True,
                    "status": res.status_code,
                    "duration_s": round(time.time() - t0, 2),
